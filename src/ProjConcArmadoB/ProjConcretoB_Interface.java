...
public class ProjConcretoB_Interface extends JFrame 
{
   ...
   //************************************************************************  
    ...
    (
      new ActionListener()
      {//Exibe uma caixa de diálogo de mensagem quando o usuário selecionar NOVO
        ...
        { // Exibe uma caixa de diálogo de mensagem quando o usuário selecionar SALVAR
            public void actionPerformed(ActionEvent event) // Método obrigatório
            {
               ...
               { // O nome do arquivo existe
                 String AvisoArquivo = JOptionPane.showInputDialog
...
//----------->>>>>>>> TRATAMENTO DE EVENTOS DO MENU ARQUIVO: 1*iTEM = "LimitesTela"------<<<<<<<<<<<<<       
       ...
       for(int i=0; i<4; i++)
       { // CampoLimites[i]= new JTextField("",4);          
...
//************ PAINEL CENTRO ***************************
    ...
    XXimin= 0.0;   XXimax=180.0 ;   YYimin=0.0 ; YYimax=100.0 ;
    desenhar.LimitesXimin(XXimin);
    desenhar.LimitesXimax(XXimax);
    desenhar.LimitesYimin(YYimin);
    desenhar.LimitesYimax(YYimax);      
...
// KATRI *****************   DATA:  10/08/21 ******************* 
 // **** KATRI ***** DATA: 17/08/21 **************** 
// Data: 28-07-20 *****************************************
    metroTamanho.addActionListener
    ( // Abre parenteses = início da chamada do addActionListener  
        new ActionListener()
        // Corpo da classe ANONIMA interna private
...
/****** ESTRUTURA VERTICAL DO PAINEL PRINCIPAL DE PROPRIEDADES DAS SEÇÕES *****/    
...
// ******************** PAINEL PROPRIEDADES DO MATERIAL ***************
      ...
      LabelMaterial = new JLabel[9]; //Vetor de Label¨
      
      LabelMaterial[0] = new JLabel("Material:");
      LabelMaterial[1] = new JLabel("fck = ");
      LabelMaterial[1].setToolTipText("Após digitar tecle ENTER!");
      LabelMaterial[2] = new JLabel("MPa");
      
      LabelMaterial[3] = new JLabel("fcd = ");
      LabelMaterial[3].setToolTipText("Após digitar tecle ENTER!");
      LabelMaterial[4] = new JLabel("MPa");
      
      LabelMaterial[5] = new JLabel("fyd = ");
      LabelMaterial[5].setToolTipText("Após digitar tecle ENTER!");
      LabelMaterial[6] = new JLabel("MPa");
      
      LabelMaterial[7] = new JLabel("fyd = ");
      LabelMaterial[7].setToolTipText("Após digitar tecle ENTER!");
      LabelMaterial[8] = new JLabel("MPa");
      
      // Cria a caixa de texto branca
      JTextField CampoMaterial[];
      CampoMaterial = new JTextField[4];
      for (int i=0; i<4; i++)
      {
        CampoMaterial[i] = new JTextField("",4);
        // CampoMaterial[i].setEditable(false);
      }
       
      // Cria os painéis a serem colocados na estrutura vertical
      JPanel painelMaterial[]; // Vetor de painéis
      painelMaterial = new JPanel[5]; // 5 painéis, 5 linhas de painéis
      
      // Coloca os JLabel e caixas de texto alinhados à esquerda
      FlowLayout layoutpainelMaterial[];
      layoutpainelMaterial = new FlowLayout[5];
      
      
      // Data: 16/07/2021
      for(int i=0; i<5; i++)
      {
          painelMaterial[i] = new JPanel(); // Criou os elementos do vetor
          layoutpainelMaterial[i]= new FlowLayout();
          
          layoutpainelMaterial[i].setAlignment(FlowLayout.LEFT); // Posicionamento do painel à esquerda
    
          painelMaterial[i].setLayout(layoutpainelMaterial[i]);
    
          //Cor personalizada nos painéis
      ...
      // ************ 1a linha - MATERIAL ************
      painelMaterial[0].add(LabelMaterial[0]);

      // *********************** 2a linha - FCK ************************
      // fck= ---- MPa (JLabel + JTextField + JLabel)
      painelMaterial[1].add(LabelMaterial[1]); // fck
      painelMaterial[1].add(CampoMaterial[0]); // Campo para digitar
      painelMaterial[1].add(LabelMaterial[2]); // MPa
      
      // *********************** 3a linha - FCD ************************
      painelMaterial[2].add(LabelMaterial[3]); // fcd
      painelMaterial[2].add(CampoMaterial[1]); // Campo para digitar
      painelMaterial[2].add(LabelMaterial[4]); // MPa
      
      // *********************** 4a linha - FYD ************************
      painelMaterial[3].add(LabelMaterial[5]); // fyd
      painelMaterial[3].add(CampoMaterial[2]); // Campo para digitar
      painelMaterial[3].add(LabelMaterial[6]); // MPa
      
      // *********************** 5a linha - FYD ************************
      painelMaterial[4].add(LabelMaterial[7]); // fyd
      painelMaterial[4].add(CampoMaterial[3]); // Campo para digitar
      painelMaterial[4].add(LabelMaterial[8]); // MPa
...
// KATRI ******* DATA: 24/08/21 ********************  
   ...
   // Cria o painelBoxMaterial
   JPanel painelBoxMaterial = new JPanel();
   painelBoxMaterial.setBorder(new LineBorder(Color.white));
   // painelBoxSeção.setBackground(new java.awt.Color(255,0,0)); // Vermelho
   painelBoxMaterial.setBackground(new java.awt.Color(217,222,231));
   
    //Cria uma estrutura vertical para colocar os 5 painéis
    Box verticalMaterial;
    verticalMaterial = Box.createVerticalBox();
    // verticalMaterial.add(Box.createVerticalStrut(20));
    verticalMaterial.add(Box.createVerticalStrut(0));
    verticalMaterial.setBorder(new LineBorder(Color.gray));

    
    verticalMaterial.add(painelMaterialInv);
    //Adiciona os paineis na estrutura vertical
    for (int i= 0; i<5;i++)
    {  
       verticalMaterial.add(painelMaterial[i]);
    }
    
    // Adiciona a estrutura "verticalMaterial" ao painelBoxSeção
    painelBoxMaterial.add(verticalMaterial);
   

    //<<<<<<<<<<<<<<<<<<<------ Adiciona painelBoxMaterial na janela (oeste = WEST à esquerda)
    ...
    // ******************** ESFORÇOS SOLICITANTES - BOX VERTICAL ************************
      JLabel LabelSolicitação[]; 
      LabelSolicitação = new JLabel[5]; //Vetor de Label¨
      
      LabelSolicitação [0] = new JLabel("Esforços solicitantes:");
      LabelSolicitação [1] = new JLabel("Msd = ");
      LabelSolicitação [1].setToolTipText("Após digitar tecle ENTER!");
      LabelSolicitação [2] = new JLabel("MPa");
      
      LabelSolicitação [3] = new JLabel("µsd =  ");
      LabelSolicitação [3].setToolTipText("Após digitar tecle ENTER!");

      
      
      // Cria a caixa de texto branca
      JTextField CampoSolicitação[];
      CampoSolicitação = new JTextField[4];
      for (int i=0; i<4; i++)
      {
        CampoSolicitação [i] = new JTextField("",4);
        // CampoSolicitação [i].setEditable(false);
      }       
      //Cria os painéis a serem colocados na estrutura vertical
      JPanel painelSolicitação[]; // Vetor de painéis
      painelSolicitação = new JPanel[4]; // 5 painéis, 5 linhas de painéis
      
      // Coloca os JLabel e caixas de texto alinhados à esquerda
      FlowLayout layoutpainelSolicitação[];
      layoutpainelSolicitação = new FlowLayout[4];
      
      
      for(int i=0; i<4; i++)
      {
          painelSolicitação [i] = new JPanel(); // Criou os elementos do vetor
          layoutpainelSolicitação [i]= new FlowLayout();
          
          // Posicionamento do painel à esquerda
          layoutpainelSolicitação [i].setAlignment(FlowLayout.LEFT);     
      ...
      // ************ 1a linha - SOLICITAÇÃO ************
      painelSolicitação [0].add(LabelSolicitação [0]);

      // *********************** 2a linha - fck ************************
      // fck= ---- MPa (JLabel + JTextField + JLabel)
      painelSolicitação [1].add(LabelSolicitação [1]); // Msd
      painelSolicitação [1].add(CampoSolicitação [0]); // Campo para digitar
      painelSolicitação [1].add(LabelSolicitação [2]); // MPa
      
      // *********************** 3a linha - µsd ************************
      painelSolicitação [2].add(LabelSolicitação [3]); // µsd
      painelSolicitação [2].add(CampoSolicitação [1]); // Campo para digitar
...
/******************************************************************************/       
      ...
      // Cria o painelBoxSolicitação
      JPanel painelBoxSolicitação = new JPanel();
      painelBoxSolicitação.setBorder(new LineBorder(Color.white));
      painelBoxSolicitação.setBackground(new java.awt.Color(217,222,231));
   
      //Cria uma estrutura vertical para colocar os 5 painéis
      Box verticalSolicitação;
      verticalSolicitação = Box.createVerticalBox();
      // verticaSolicitação.add(Box.createVerticalStrut(20));
      verticalSolicitação.add(Box.createVerticalStrut(0));
      verticalSolicitação.setBorder(new LineBorder(Color.gray));

      verticalSolicitação.add(painelSolicitaçãoInv);

      //Adiciona os paineis na estrutura vertical
      for (int i= 0; i<4;i++)
      {  
         verticalSolicitação.add(painelSolicitação[i]);
      }    
      // Adiciona a estrutura "verticalSolicitação" ao painelBoxSeção
      painelBoxSolicitação.add(verticalSolicitação);   

      //<<<<<<<<<<<<<<<<<<<------ Adiciona painelBoxSolicitação na janela (oeste = WEST à esquerda)
...
/****** PAINEL QUE APRESENTA AS IMAGENS DOS TIPOS DE DETALHAMENTOS*******************/    
...
//**************************************************************************************
    ...
    /****** TRATAMENTO DE EVENTO: BOTÃO PARA CALCULAR PROPRIEDADES GEOMÉTRICAS***********/    
        FlagSeção = 0; // ?????????????????????????????????
